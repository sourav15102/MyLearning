...
- Rules:
	- If parent method throws X exception, child's overridden method can only throw either same X or subclass of X, but never parent of X., number of exception should be same as well.
...
A: 
No, They are not inherited so, cant be final.
...
**Q: main method things** [[003 - main method]]
A: 
...
Q: classLoader.loadClass vs Class.forName
A:
**Clearing Your Confusion: Class Loading vs. Initialization in Java**
1. **Class Loading** → Loads the class **but does NOT run static blocks**.
2. **Class Initialization** → Runs **static blocks and initializes static variables**.
3. **Object Instantiation** → Calls **the constructor** (after initialization).

`Class.forName()` returns a `java.lang.Class` object. Think of this `Class` object as a _representation_ of the class containing metadata of the class you're trying to load. It's not the class itself, but an object that _describes_ the class.

---
**1️⃣ Class Loading (`ClassLoader.loadClass()`)**
🔹 **Loads the class but does NOT initialize it (i.e., no static block execution).**
**Example**
...
public class LoadClassExample {
    public static void main(String[] args) throws Exception {
        ...
        classLoader.loadClass("Test"); // Class is loaded but NOT initialized
    }
}
```

✅ **Output:** _(Nothing prints!)_  
**Why?** Because `loadClass("Test")` only **loads the class** but does NOT execute the static block.

---
**2️⃣ Class Initialization (`Class.forName()`)**
🔹 **Loads and initializes the class (runs static blocks).**
 **Example**

```java
public class ForNameExample {
    public static void main(String[] args) throws Exception {
        Class.forName("Test"); // Class is loaded AND initialized
    }
}
```
...
- **Class is loaded AND initialized**, so the static block runs.

---

**3️⃣ Object Instantiation (Constructor Execution)**

🔹 The **constructor only runs when an object is created**.
 **Example**
...
public class InstantiateExample {
    public static void main(String[] args) throws Exception {
        ...
        Object obj = clazz.getDeclaredConstructor().newInstance(); // Calls constructor
    }
}
```
...
4. `Class.forName("Test")` **loads & initializes** the class → Static block runs.
5. `newInstance()` **creates an object** → Constructor runs.

> ## **1️⃣ What is Class Loading?**
**Class Loading** is the process where the **JVM loads the `.class` file** (bytecode) into **memory (Method Area)** before execution.
 **2️⃣ What is Class Initialization?**
Once a class is loaded, **class initialization happens**, which includes:
- Running **static blocks**.
- Assigning **explicit values** to static variables.


Q: Regarding checked exceptions, return type and access modifier give me how we an change it while overriding a method and overloading.
A:
overriding:
- checked exceptions can be same or less in number and can be same or subclass.
- unchecked no rule
- return type can be sane or (subtype: Covariant)
- access modifier can be same or liberating: private (cant be overridden)
	- default -> protected -> public
Overloading:
- No rules

### **1️⃣ `Collections.synchronizedList()`**

- **Creates a thread-safe wrapper around a list**, ensuring synchronized access.
- **Example:**
...
- **Hints the JVM scheduler to pause the current thread and allow others to run.**
- **Example:**
...
✅ **May or may not pause execution, depends on JVM scheduling.**

---

### **3️⃣ `Collections.unmodifiableList()`**
...
✅ **Prevents modifications, ensuring safety in read-only scenarios.**

---

### **4️⃣ `AtomicInteger.incrementAndGet()`**

- **Atomically increments an integer and returns the new value (thread-safe).**
- **Example:**
    ...
    public class AtomicExample {
        public static void main(String[] args) {
            ...
            System.out.println(counter.incrementAndGet()); // Output: 2
        }
    }
    ```
    

✅ **Ensures atomic operations without explicit synchronization.**

---

### **5️⃣ `Enum.valueOf()`**

- **Retrieves an enum constant by its name (case-sensitive).**
- **Example:**
...
class MyString {
    ...
    public boolean equals(Object obj) {
        ...
        return Objects.equals(value, myStr.value); // Compare actual string values
    }

    // Override hashCode() to generate consistent hash
    @Override
    public int hashCode() {
        return Objects.hash(value); // Uses the internal hashCode of the String
    }
...
public class HashCodeEqualsExample {
    public static void main(String[] args) {
        ...
        System.out.println(str1.equals(str3)); // ❌ false (different content)
        System.out.println(str1.hashCode()); // Same hash for equal values
        System.out.println(str2.hashCode()); // Same as str1
        System.out.println(str3.hashCode()); // Different for "world"
    }
}

```

### VisualVM MAT and GC Viewer
visual VM is a free tool within the jdk for monitoring Java. Applications it captures, heap and garbage collection dumps which are binary snapshots of memory usage and garbage collection activity. You can analyze these dumps using tools like Eclipse, memory Analyzer Tool (MAT), for Heap dumps, and GC viewer for garbage collection dumps
1. Get heap memory dump from Visual VM, adn analyze them using memory Analyzer Tool (MAT) for memory leaks
2. Connect to Java Applicaiton in Visual VM
3. Create dump of GC
4. Anaytlze that dump in GC viewer